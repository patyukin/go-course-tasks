...
func worker(ctx context.Context, wg *sync.WaitGroup, tasks <-chan DownloadTask, results chan<- error, concurrencyLimiter chan struct{}) {
	defer wg.Done()
	...
	for task := range tasks {
		...
		// Acquire token to limit concurrency
		concurrencyLimiter <- struct{}{}
		err := downloadFile(ctx, task)
		results <- err
		// Release token after completing task
		<-concurrencyLimiter
	}
}
